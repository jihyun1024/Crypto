...
void big_int_to_hex_string(const BigInt *a, char *hex_str, int max_len) {
    ...
    for (int i = NUM_LIMBS - 1; i >= 0 && pos < max_len - 1; i--) {
        ...
        for (int j = 15; j >= 0 && pos < max_len - 1; j--) {
            int nibble = (limb >> (j * 4)) & 0xF;
            if (nibble != 0 || found_nonzero) {
                hex_str[pos++] = "0123456789ABCDEF"[nibble];
                found_nonzero = 1;
            }
        }
    }
...
void big_int_print(const BigInt *a) {
    char hex_str[NUM_LIMBS * 16 + 1];
    big_int_to_hex_string(a, hex_str, sizeof(hex_str));
    printf("%s", hex_str);
}
